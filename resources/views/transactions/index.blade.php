    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('transactions.trash') }}" class="inline-flex items-center px-4 py-2 bg-gray-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700">
                ...
                <div class="p-6">
                    ...
                    @else
                        <div class="space-y-3">
                            ...
                            <div class="border border-gray-200 rounded-lg overflow-hidden {{ ($group['is_dividend'] ?? false) ? 'ring-2 ring-purple-300' : '' }} {{ ($group['is_maintenance'] ?? false) ? 'ring-2 ring-orange-300' : '' }} {{ ($group['is_fund_deposit'] ?? false) ? 'ring-2 ring-blue-300' : '' }}">
                                {{-- Header - Click để mở rộng --}}
                                <div class="px-4 py-3 cursor-pointer hover:bg-gray-100 transition {{ ($group['is_dividend'] ?? false) ? 'bg-purple-50' : (($group['is_maintenance'] ?? false) ? 'bg-orange-50' : (($group['is_fund_deposit'] ?? false) ? 'bg-blue-50' : 'bg-gray-50')) }}" onclick="toggleDetail('detail-{{ $loop->index }}')">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center space-x-4">
                                            ...
                                            <div>
                                                ...
                                                @elseif($group['is_fund_deposit'] ?? false)
                                                    <span class="text-base font-semibold text-blue-600">📦 Nộp quỹ</span>
                                            ...
                                            <div class="flex items-center space-x-3 text-sm text-gray-600">
                                                ...
                                                @elseif($group['is_fund_deposit'] ?? false)
                                                    <span class="text-blue-600">({{ $group['transactions']->count() }} giao dịch)</span>
                                                @elseif($group['incident'])
                                ...
                                {{-- Chi tiết giao dịch --}}
                                <div id="detail-{{ $loop->index }}" class="hidden {{ ($group['is_dividend'] ?? false) ? 'bg-purple-50' : (($group['is_maintenance'] ?? false) ? 'bg-orange-50' : (($group['is_fund_deposit'] ?? false) ? 'bg-blue-50' : 'bg-white')) }}">
                                            ...
                                            <tbody class="divide-y divide-gray-100">
                                                ...
                                                <tr class="hover:bg-gray-50 {{ $transaction->category == 'điều_chỉnh_lương' ? 'bg-blue-50' : '' }} {{ ($group['is_dividend'] ?? false) ? 'bg-purple-50' : '' }} {{ ($group['is_maintenance'] ?? false) ? 'bg-orange-50' : '' }} {{ ($group['is_fund_deposit'] ?? false) ? 'bg-blue-50' : '' }}">
                                                    ...
                                                    @elseif($group['is_dividend'] ?? false)
                                                        ...
                                                        <td class="py-2">
                                                            @php
                                                                // Extract shareholder name from note: "Chia cổ tức X% - Tên cổ đông (Vốn góp: Y%)"
                                                                preg_match('/- ([^(]+) \(Vốn góp/', $transaction->note ?? '', $matches);
                                                                $shareholderName = $matches[1] ?? 'N/A';
                                                            @endphp
                                                    ...
                                                    @if($group['is_fund_deposit'] ?? false)
                                                        ...
                                                        <td class="py-2">
                                                            @if($transaction->vehicle_id)
                                                                <a href="{{ route('vehicles.show', $transaction->vehicle_id) }}" class="text-blue-600 hover:text-blue-800 font-medium">
                                                                    🚗 {{ $transaction->vehicle->license_plate ?? 'Xe #'.$transaction->vehicle_id }}
                                                                </a>
                                                                @if($transaction->vehicle && $transaction->vehicle->hasOwner())
                                                                    <span class="text-xs text-blue-600 ml-1">(Có chủ - vào số dư xe)</span>
                                                                @else
