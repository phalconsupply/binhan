                    ...
                    <form method="POST" action="{{ route('assets.update', $asset) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="date" name="equipped_date" id="equipped_date" value="{{ old('equipped_date', $asset->equipped_date->format('Y-m-d')) }}" required
                            ...
                            <div>
                                ...
                                <input type="number" name="quantity" id="quantity" value="{{ old('quantity', $asset->quantity) }}" min="1" required
                            ...
                            <div>
                                ...
                                <select name="usage_type" id="usage_type" required onchange="toggleUsageFields()"
                                    ...
                                    <option value="vehicle" {{ old('usage_type', $asset->usage_type) == 'vehicle' ? 'selected' : '' }}>🚗 Xe</option>
                                    <option value="staff" {{ old('usage_type', $asset->usage_type) == 'staff' ? 'selected' : '' }}>👤 Cá nhân</option>
                                </select>
                                @error('usage_type')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <select name="is_active" id="is_active"
                                    ...
                                    <option value="1" {{ old('is_active', $asset->is_active) == 1 ? 'selected' : '' }}>✓ Đang sử dụng</option>
                                    <option value="0" {{ old('is_active', $asset->is_active) == 0 ? 'selected' : '' }}>✕ Ngừng sử dụng</option>
                                </select>
                            </div>
                            ...
                            <div id="vehicle_field" style="display: none;">
                                ...
                                <select name="vehicle_id" id="vehicle_id"
                                    ...
                                    @foreach($vehicles as $vehicle)
                                        <option value="{{ $vehicle->id }}" {{ old('vehicle_id', $asset->vehicle_id) == $vehicle->id ? 'selected' : '' }}>
                            ...
                            <div id="staff_field" style="display: none;">
                                ...
                                <select name="staff_id" id="staff_id"
                                    ...
                                    @foreach($staffs as $staff)
                                        <option value="{{ $staff->id }}" {{ old('staff_id', $asset->staff_id) == $staff->id ? 'selected' : '' }}>
                        ...
                        <div class="mt-6">
                            ...
                            <textarea name="note" id="note" rows="3"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('note') border-red-300 @enderror">{{ old('note', $asset->note) }}</textarea>
                            @error('note')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
